...
export default function ArsivPage() {
  ...
  const loadDocuments = async () => {
    try {
      ...
      if (result && result.success) {
        ...
        // API başarısız oldu
        const errorMessage =
          result?.message || "Belgeler yüklenirken hata oluştu";
  ...
  const handleFileUpload = async (documentType, file, allowExcel = false) => {
    ...
    if (
      ...
      setError(
        allowExcel
          ? "Sadece PDF ve Excel dosyaları yüklenebilir"
    ...
    try {
      ...
      if (result.success) {
        const docName =
          DOCUMENT_TYPES.find((d) => d.id === documentType)?.name || "Belge";
