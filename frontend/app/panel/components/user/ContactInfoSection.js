...
export default function ContactInfoSection({ contactInfos, userId, onUpdate, isApproved }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      {showForm && (
        <form onSubmit={isEditing ? (e) => { e.preventDefault(); handleUpdate(contactInfos[0]?.id); } : handleAdd} className="mb-6 p-4 bg-gray-50 rounded-lg space-y-4">
          <div>
            ...
            <div key={contact.id} className="border border-gray-200 rounded-lg p-4">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600"><span className="font-medium">E-posta:</span> {contact.mail || '-'}</p>
                </div>
